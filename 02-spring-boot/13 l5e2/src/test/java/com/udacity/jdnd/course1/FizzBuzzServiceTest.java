...
class FizzBuzzServiceTest {
	...
	void testBuzzFizz(){
		...
		//check converting input to Integer
		assertThrows(NumberFormatException.class,() -> fbs.buzzFizz("Text",12));

		// check valid Fizz
		assertEquals(3 * 4,fbs.buzzFizz("Fizz",4));

		//check valid Buzz
		assertEquals(5*7, fbs.buzzFizz("Buzz",7));

		//check valid FizzBuzz
		assertEquals(15*2,fbs.buzzFizz("FizzBuzz",2));
